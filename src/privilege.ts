 ...
 * scheduled delivery is the active agent turn (until OMP agent_end).
 *
...
export class PrivilegeGuard {
  ...
  attach(pi: ExtensionAPI): void {
    pi.on("tool_call", async (event) => {
      ...
      if (tier === "read_only") {
        if (MUTATE_TOOLS.has(name)) {
          return {
            block: true,
            reason: `[omp-schedule] blocked ${name}: active scheduled job is tier=read_only`,
          };
        }
        if (scheduleMut) {
          return {
            block: true,
            reason:
              `[omp-schedule] blocked schedule ${String((event as { input?: { action?: unknown } }).input?.action)}: ` +
      ...
      if (tier === "suggest") {
        if (SUGGEST_BLOCK.has(name)) {
          return {
            block: true,
            reason: `[omp-schedule] blocked ${name}: active scheduled job is tier=suggest (no shell)`,
          };
        }
        if (scheduleMut) {
          return {
    ...
    pi.on("agent_end", async (event) => {
      // Auto-retry/continuation is still the same logical scheduled run.
