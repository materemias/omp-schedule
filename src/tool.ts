...
import type { TypeBuilder } from "@oh-my-pi/pi-coding-agent/extensibility/legacy-typebox";
import {
  ActionError,
...
function buildScheduleParams(Type: TypeBuilder) {
  return Type.Object({
  action: Type.Enum([
    "create",
    "list",
   ...
   */
  kind: Type.Optional(
    Type.Enum(["prompt", "shell", "notify", "message"] as const),
  ),
  ...
  wakeOn: Type.Optional(
    Type.Enum(["always", "failure", "success", "never"] as const),
  ),
  ...
  tier: Type.Optional(
    Type.Enum(["read_only", "suggest", "mutate"] as const),
  ),
...
/**
 * OpenAI Responses strict tool schemas require every property to be present.
 * OMP therefore receives neutral placeholders for fields that are logically
 * optional ("", 0, and an enum member). Strip those placeholders before
 * applying the action-specific validation rules.
 */
function normalizeStrictCreateParams(params: {
  name?: string;
  kind?: string;
  prompt?: string;
  command?: string;
  ...
  return {
    ...
    kind,
    prompt: nonEmpty(params.prompt),
    command: shell ? nonEmpty(params.command) : undefined,
    wakeOn: shell ? nonEmpty(params.wakeOn) : undefined,
    successPrompt: shell ? nonEmpty(params.successPrompt) : undefined,
    failurePrompt: shell ? nonEmpty(params.failurePrompt) : undefined,
    timeoutMs:
      shell && params.timeoutMs !== undefined && params.timeoutMs > 0
        ? params.timeoutMs
        : undefined,
    once: nonEmpty(params.once),
    every: nonEmpty(params.every),
    dailyAt: nonEmpty(params.dailyAt),
    maxRuns: params.maxRuns === 0 ? undefined : params.maxRuns,
  };
}
...
export function registerScheduleTool(
  ...
  const ScheduleParams = buildScheduleParams(pi.typebox.Type);
...
function handleCreate(
  ...
  const normalized = normalizeCreateAction({
    ...
    wakeOn: clean.wakeOn,
    successPrompt: clean.successPrompt,
    failurePrompt: clean.failurePrompt,
  ...
  const tier: PrivilegeTier = normalized.forceTierMutate
    ? "mutate"
    : (clean.tier ?? DEFAULT_TIER);
  const maxRuns = normalizeMaxRuns(clean.maxRuns);

  // Invalid calls do not consume the creation budget. This matters with
  // strict-schema retries, where one logical request may be reformulated.
