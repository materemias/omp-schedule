 ...
 * nothing. OMP agent_end pops the stack one level (follow-up discipline).
 *
...
function setup() {
  let toolCall:
    ...
    | null = null;
  let settled: ((event: { willContinue?: boolean }) => Promise<void>) | null = null;
  const pi = {
    on(
      event: string,
      handler:
        ...
        | ((event: { willContinue?: boolean }) => Promise<void>),
    ): void {
      if (event === "tool_call") toolCall = handler as typeof toolCall;
      if (event === "agent_end") settled = handler as typeof settled;
    },
  ...
  return {
    ...
    settle: (willContinue = false) => settled!({ willContinue }),
  };
}
...
describe("PrivilegeGuard tier enforcement", () => {
  ...
  it("OMP agent_end clears exactly one scheduled turn", async () => {
  ...
  it("OMP agent_end keeps the guard during automatic continuation", async () => {
    const { guard, call, settle } = setup();
    guard.enter("read_only");
    await settle(true);
