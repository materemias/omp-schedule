# omp-schedule reliability model
...
**Mitigation — append-only run ledger** `~/.omp/schedule/runs.jsonl`:
...
|------|--------|---------------------------|
| `read_only` | no mutations | blocks `edit`, `write`, `bash` until final OMP `agent_end` |
...
| Project root | `.omp/schedule.json` under **`ctx.cwd` only** — no upward walk. Launch OMP from project root |
...
<project>/.omp/schedule.json   # project jobs
```
